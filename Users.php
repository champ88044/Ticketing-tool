...
class Users extends Database { 
	...
	public function login(){		
		...
		if(!empty($_POST["login"]) && $_POST["email"]!=''&& $_POST["password"]!='') {	
			...
			if($isValidLogin){
				...
				else if ($userDetails['user_type'] == 'engineer') {
                    $_SESSION["engineer"] = 1;  // Setting the engineer session variable
	...
	public function listUser(){
		...
		while( $user = mysqli_fetch_assoc($result) ) {		
			...
			if($user['user_type'] == 'admin')	{
				...
				$userRole = '<span class="label label-warning">Engineer</span>';
			}	
	...
	public function getUserPassword($userId) {
        $query = "SELECT password FROM ".$this->userTable." WHERE id='$userId'";
        $result = $this->dbConnect->query($query);
        if ($result === false) {
            error_log("Database query error: " . $this->dbConnect->error);
            return false;
        }
	...
	public function updatePassword($userId, $newPassword) {
		...
		$sqlQuery = "UPDATE ".$this->userTable." 
