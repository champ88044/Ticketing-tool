...
class Tickets extends Database {  
	...
	// 	$sqlWhere = '';	
	// 	// if(!isset($_SESSION["admin"])   && !isset($_SESSION["engineer"])) {
	// 	// 	$sqlWhere .= " WHERE t.user = '".$_SESSION["userid"]."' ";
	// 	// 	if(!empty($_POST["search"]["value"])){
	// 	// 		$sqlWhere .= " AND ";
	// 	// 	}
	// 	// } else if((isset($_SESSION["admin"]) ||  isset($_SESSION["engineer"]) && !empty($_POST["search"]["value"])) {
	...
	// 	$time = new time;  			 
	// 	$sqlQuery = "SELECT t.id, t.uniqid, t.title, t.init_msg as message, t.date, t.last_reply, t.resolved, u.name as creater, d.name as department, u.user_type, t.user, t.user_read, t.admin_read,t.engineer_read
	// 		FROM hd_tickets t 
    ...
    //         (isset($_SESSION["engineer"]) && !$ticket['engineer_read'] && $ticket['last_reply'] != $_SESSION["userid"]) || 
    //         (!isset($_SESSION["admin"]) && !isset($_SESSION["engineer"]) && !$ticket['user_read'] && $ticket['last_reply'] != $ticket['user'])) {
    //         $title = $this->getRepliedTitle($ticket['title']);			
    //         }
	...
	// 		 $ticketRows[] = '<button type="button" name="delete" id="'.$ticket["id"].'" class="btn btn-danger btn-xs delete"  '.$disb.'>Delete</button>';
	...
	public function showTickets() {
		...
		// Check if the user is not an admin or engineer and filter tickets by the logged-in user
		if (!isset($_SESSION["admin"]) && !isset($_SESSION["engineer"])) {
			$sqlWhere .= " WHERE t.user = '".$_SESSION["userid"]."' ";
			if (!empty($_POST["search"]["value"])) {
				$sqlWhere .= " AND ";
		...
		// If the user is an admin or engineer and there's a search value, add appropriate conditions
		else if ((isset($_SESSION["admin"]) || isset($_SESSION["engineer"])) && !empty($_POST["search"]["value"])) {
			if (strpos($sqlWhere, 'WHERE') === false) {
				$sqlWhere .= " WHERE ";
			} else {
		...
		while ($ticket = mysqli_fetch_assoc($result)) {        
			...
			// Set $disabled for the edit button based on user roles
			$disabled = '';
			if (!isset($_SESSION["admin"]) && !isset($_SESSION["engineer"])) {
				$disabled = 'disabled';
			}
			
			// Set $disb for the delete button to be disabled for non-admins and engineers
			$disb = '';
			...
			$ticketRows[] = $status;
			$ticketRows[] = '<a href="view_ticket.php?id='.$ticket["uniqid"].'" class="btn btn-success btn-xs update">View Ticket</a>';    
			$ticketRows[] = '<button type="button" name="update" id="'.$ticket["id"].'" class="btn btn-warning btn-xs update" '.$disabled.'>Edit</button>';
			$ticketRows[] = '<button type="button" name="delete" id="'.$ticket["id"].'" class="btn btn-danger btn-xs delete" '.$disb.'>Delete</button>';
			$ticketData[] = $ticketRows;
		}
	...
	public function createTicket() {      
		if (!empty($_POST['subject']) && !empty($_POST['message'])) {                
			...
			if (mysqli_query($this->dbConnect, $queryInsert)) {
				...
				echo '<div class="alert error">Error: '.mysqli_error($this->dbConnect).'</div>';
			}
		} else {
			echo '<div class="alert error">Please fill in all fields.</div>';
		}
    ...
    public function deleteTicket() {
        if ($_POST['ticketId']) {
            ...
            $isDeleted = mysqli_query($this->dbConnect, $sqlDelete);
            if ($isDeleted) {
                echo 'success';
            } else {
